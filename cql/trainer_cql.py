...
from batch_rl_algorithm import BatchRLAlgorithm
from online_rl_algorithm import OnlineRLAlgorithm
...
class CQLTrainer(TorchTrainer):
    ...
    def _get_policy_actions(self, obs, num_actions, network=None):
        ...
        new_obs_actions, _, _, new_obs_log_pi, *_ = self.policy.model_forward(
    ...
    def train_from_torch(self, batch):
        ...
        obs = obs.to(device)
        new_obs_actions, policy_mean, policy_log_std, log_pi, *_ = self.policy.model_forward(
        ...
        new_next_actions, _, _, new_log_pi, *_ = self.policy.model_forward(
            next_obs, reparameterize=True, return_log_prob=True,
        )
        new_curr_actions, _, _, new_curr_log_pi, *_ = self.policy.model_forward(
            obs, reparameterize=True, return_log_prob=True,
        )
        ...
        if self.max_q_backup:
            ...
            next_actions_temp, _ = self._get_policy_actions(next_obs, num_actions=10, network=self.policy.model)
        ...
        # a = (new_curr_actions - actions) / actions
        # mask = a > 1.2
        # a = a * mask
        # p_rewards = a * (-10) + rewards
        ...
        curr_actions_tensor, curr_log_pis = self._get_policy_actions(obs, num_actions=self.num_random, network=self.policy.model)
        new_curr_actions_tensor, new_log_pis = self._get_policy_actions(next_obs, num_actions=self.num_random, network=self.policy.model)
        ...
        if self._need_to_update_eval_statistics:
            ...
            if self.num_qs > 1:
                ...
                print(f"Policy Loss: {policy_loss}, QF1 Loss: {self.eval_statistics['QF1 Loss']}, Min QF1 Loss: {self.eval_statistics['min QF1 Loss']}, QF2 Loss: {self.eval_statistics['QF2 Loss']}, Min QF2 Loss: {self.eval_statistics['min QF2 Loss']}")
            else:
                # Print policy_loss, QF1 Loss, and min QF1 Loss (no QF2 Loss available)
                print(f"Policy Loss: {policy_loss}, QF1 Loss: {self.eval_statistics['QF1 Loss']}, Min QF1 Loss: {self.eval_statistics['min QF1 Loss']}")
            ...
            if self.use_automatic_entropy_tuning:
                ...
                print(f"Alpha: {self.eval_statistics['Alpha']}, Alpha Loss: {self.eval_statistics['Alpha Loss']}")
